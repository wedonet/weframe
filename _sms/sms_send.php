...
//require(sysdir . '/_sms/HttpSend.php');//北京华兴短信接口
require(sysdir . '/_sms/yunpiansms.php');//北京华兴短信接口
...
function insertsendLog($rs) {
	$insertid = $GLOBALS ['we']->pdo->insert ( sheet . '_sendsms', $rs );
	//return true;
}
function usersms($res,$order){
	...
	if(in_array($rs['sendtype'], $memmobilearr)){
		...
		if($order['guestmobile']!=false)$rs['sendmobile']=$order['guestmobile'];
	...
	if(isset($order['hotelid']))$rs['sendcomid']=$order['hotelid'];
	if(isset($GLOBALS ['we']->user ['id']))$rs['sendduid']= $GLOBALS ['we']->user ['id'];
	$rs['sendtimeint']=time();
	$rs['sendip']=$GLOBALS['we']->getip();
	//$dir =  sysdir.'_sms/log/'.time().'.txt';
	//$GLOBALS['we']->write_file($dir , implode(',',$rs));
	if(issendsms==1)insertsendlog($rs);
	return true;
}
function getpayrs(){
	return true;
}
function getsendmsg($rs,$order=null) {
	$delaytime = '';
	if(isset($order['hotelid']))
	    $delaytime = gettime($order['hotelid']);
	...
	switch ($type) {
		case 1 :
			$rs ['sendmsg'] = $ejialogo.'您注册e家人的激活码为：'.$order['activecode'].'，30分钟内有效，请妥善保管，' . $ejiatel;
			break;
		case 2 :
		...
		case 12 :
			...
			break;
		case 13 :
			$rs ['sendmsg'] =  $ejialogo.$ejianick .  '您正在申请找回密码，验证码为'.$order['randcode'].'，30分钟内有效，请妥善保管。如非本人操作，请及时与客服联系。' . $ejiatel;
			break;
	    case 14 :
