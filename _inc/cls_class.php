...
class Cls_Class {
	...
	function Main() {
		...
		if ('' != $this->cic) {
	     ...
	     //stop($this->channel,true);
		if($this->cic=='news')
			$title = '动态分类';
		elseif($this->cic=='help')
			$title = '帮助分类';
		else
			$title = '分类管理';
		...
		if (is_array($this->channel)) {
			...
			crumb('<a href="?cid=' . $this->cid . '">'.$title.'</a>');
		}

        //$classid = $GLOBALS['we']->rqid('classid');
        //stop($classid);
	...
	function ShowList() {
		...
		$sql = 'select * from `' . sheet . '_class` where cid=' . $this->cid;
...
// end func
	...
	function SaveForm($isedit) {
		...
		$rs['ic'] = $GLOBALS['we']->request('编码', 'ic', 'post', 'char', 1, 20, 'invalid', $isic);

		$rs['title'] = $GLOBALS['we']->request('名称', 'title', 'post', 'char', 1, 80);
		$rs['readme'] = $GLOBALS['we']->request('介绍', 'readme', 'post', 'char', 1, 500, '', FALSE);
		$rs['preimg'] = $GLOBALS['we']->request('预览图', 'preimg', 'post', 'char', 1, 255, 'encode', FALSE);
		$rs['tags'] = $GLOBALS['we']->request('关键词', 'tags', 'post', 'char', 1, 50, '', FALSE);
		...
		$rs['mystyle'] = $GLOBALS['we']->request('模板路径', 'mystyle', 'post', 'char', 1, 255, 'folder', FALSE);
...
// 			stop($rsparent,true);
		...
		try {
			...
			/* 清除分类缓存 */
			$GLOBALS['cache']->delete(CacheName.'class_'.($this->cid.'_'.$this->module));
